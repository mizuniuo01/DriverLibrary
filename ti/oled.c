...
static I2C_Regs* oled_i2c_handle = NULL;

volatile uint8_t oled_tick_flag = 0;

typedef enum
{
    OLED_TX_STATE_IDLE = 0,
    OLED_TX_STATE_BUSY
} OLED_TxState_t;

typedef enum
{
    OLED_RUN_STATE_IDLE = 0,
    OLED_RUN_STATE_INIT,
    OLED_RUN_STATE_UPDATE,
    OLED_RUN_STATE_CURSOR
} OLED_RunState_t;

typedef enum
{
    OLED_UPDATE_STEP_SET_PAGE = 0,
    OLED_UPDATE_STEP_SET_COL_HIGH,
    OLED_UPDATE_STEP_SET_COL_LOW,
    OLED_UPDATE_STEP_SEND_DATA
} OLED_UpdateStep_t;

typedef enum
{
    OLED_CURSOR_STEP_PAGE = 0,
    OLED_CURSOR_STEP_COL_HIGH,
    OLED_CURSOR_STEP_COL_LOW
} OLED_CursorStep_t;

static OLED_TxState_t oledTxState = OLED_TX_STATE_IDLE;
static OLED_RunState_t oledRunState = OLED_RUN_STATE_IDLE;
static OLED_UpdateStep_t oledUpdateStep = OLED_UPDATE_STEP_SET_PAGE;
static OLED_CursorStep_t oledCursorStep = OLED_CURSOR_STEP_PAGE;

static uint16_t oledInitIndex = 0;
static uint8_t oledUpdateStartPage = 0;
static uint8_t oledUpdateEndPage = 0;
static uint8_t oledUpdatePage = 0;
static uint8_t oledUpdateX = 0;
static uint8_t oledUpdateWidth = 0;

static uint8_t oledUpdatePending = 0;
static uint8_t oledPendingStartPage = 0;
static uint8_t oledPendingEndPage = 0;
static uint8_t oledPendingX = 0;
static uint8_t oledPendingWidth = 0;

static uint8_t oledCursorPending = 0;
static uint8_t oledCursorPage = 0;
static uint8_t oledCursorX = 0;

static uint8_t oledTxControlByte = 0x00;
static uint8_t oledTxSingleByte = 0;
static const uint8_t *oledTxData = NULL;
static uint16_t oledTxCount = 0;
static uint16_t oledTxIndex = 0;

static const uint8_t oledInitCmds[] = {
    0xAE, 0xD5, 0x80, 0xA8, 0x3F, 0xD3, 0x00, 0x40,
    0xA1, 0xC8, 0xDA, 0x12, 0x81, 0xCF, 0xD9, 0xF1,
    0xDB, 0x30, 0xA4, 0xA6, 0x8D, 0x14, 0xAF
};

static bool OLED_TxStart(uint8_t controlByte, const uint8_t *data, uint16_t count);
static void OLED_TxPoll(void);
static void OLED_ProcessInit(void);
static void OLED_ProcessUpdate(void);
static void OLED_ProcessCursor(void);
static void OLED_RequestUpdate(uint8_t startPage, uint8_t endPage, uint8_t startX, uint8_t width);
...
static bool OLED_TxStart(uint8_t controlByte, const uint8_t *data, uint16_t count)
{
    if (oled_i2c_handle == NULL || data == NULL || count == 0)
    {
        return false;
    }

    if (oledTxState != OLED_TX_STATE_IDLE)
    {
        return false;
    }

    if (!(DL_I2C_getControllerStatus(oled_i2c_handle) & DL_I2C_CONTROLLER_STATUS_IDLE))
    {
        return false;
    }

    oledTxControlByte = controlByte;
    oledTxData = data;
    oledTxCount = count;
    oledTxIndex = 0;

    DL_I2C_startControllerTransfer(oled_i2c_handle, OLED_I2C_ADDRESS >> 1,
    DL_I2C_CONTROLLER_DIRECTION_TX, count + 1);

    if (!DL_I2C_isControllerTXFIFOFull(oled_i2c_handle))
    {
        DL_I2C_fillControllerTXFIFO(oled_i2c_handle, &oledTxControlByte, 1);
    }

    oledTxState = OLED_TX_STATE_BUSY;
    OLED_TxPoll();
...
static void OLED_TxPoll(void)
{
    if (oled_i2c_handle == NULL || oledTxState != OLED_TX_STATE_BUSY)
    {
        return;
    }

    if (DL_I2C_getControllerStatus(oled_i2c_handle) & DL_I2C_CONTROLLER_STATUS_ERROR)
    {
        OLED_ErrorCallback(oled_i2c_handle);
        return;
    }

    while (oledTxIndex < oledTxCount && !DL_I2C_isControllerTXFIFOFull(oled_i2c_handle))
    {
        DL_I2C_fillControllerTXFIFO(oled_i2c_handle, &oledTxData[oledTxIndex], 1);
        oledTxIndex++;
    }

    if (oledTxIndex >= oledTxCount)
    {
        if (DL_I2C_getControllerStatus(oled_i2c_handle) & DL_I2C_CONTROLLER_STATUS_IDLE)
        {
            oledTxState = OLED_TX_STATE_IDLE;
        }
    }
}
...
void OLED_Task(void)
{
    float leftSpeedMmS = 0.0f, rightSpeedMmS = 0.0f;
    int8_t bt_num1 = 0, bt_num2 = 0;
    uint8_t bt_used = 0;
    Blueteeth_GetOledDigit(&bt_num1, &bt_num2, &bt_used);

    Trace_Get_RealSpeed(&leftSpeedMmS, &rightSpeedMmS);
    OLED_ShowFloatNum(0, 20, leftSpeedMmS, 3, 1, OLED_8X16);
    OLED_ShowString(50, 20, "mm/s", OLED_8X16);
    OLED_ShowFloatNum(0, 35, rightSpeedMmS, 3, 1, OLED_8X16);
    OLED_ShowString(50, 35, "mm/s", OLED_8X16);
    OLED_ShowString(0, 50, "Num:", OLED_8X16);
    if (bt_used)
    {
        OLED_ShowNum(35, 50, (uint32_t)bt_num1, 1, OLED_8X16);
        OLED_ShowNum(50, 50, (uint32_t)bt_num2, 1, OLED_8X16);
    }
    else
    {
        OLED_ShowNum(35, 50, (uint32_t)camData.detectNum1, 1, OLED_8X16);
        OLED_ShowNum(50, 50, (uint32_t)camData.detectNum2, 1, OLED_8X16);
    }

    OLED_Update();
    // 定时刷新标志位以定时控制oled刷新
    if (oled_tick_flag == 0) return;
    oled_tick_flag = 0;
    ...
    if (oledRunState == OLED_RUN_STATE_IDLE)
    {
        if (oledUpdatePending != 0)
        {
            oledUpdatePending = 0;
            OLED_RequestUpdate(oledPendingStartPage, oledPendingEndPage, oledPendingX, oledPendingWidth);
            return;
        }

        if (oledCursorPending != 0)
        {
            oledRunState = OLED_RUN_STATE_CURSOR;
        }
    }

    switch (oledRunState)
    {
        case OLED_RUN_STATE_INIT:
        OLED_ProcessInit();
        break;
        case OLED_RUN_STATE_UPDATE:
        OLED_ProcessUpdate();
        break;
        case OLED_RUN_STATE_CURSOR:
        OLED_ProcessCursor();
        break;
        default:
        break;
...
static void OLED_ProcessInit(void)
{
    if (oledInitIndex >= sizeof(oledInitCmds))
    {
        oledRunState = OLED_RUN_STATE_IDLE;
    ...
    if (OLED_TxStart(0x00, &oledInitCmds[oledInitIndex], 1))
    {
        oledInitIndex++;
    }
}
...
static void OLED_ProcessUpdate(void)
{
    ...
    if (oledUpdatePage > oledUpdateEndPage)
    {
        oledRunState = OLED_RUN_STATE_IDLE;
    ...
    switch (oledUpdateStep)
    {
        case OLED_UPDATE_STEP_SET_PAGE:
        cmd = 0xB0 | oledUpdatePage;
        oledTxSingleByte = cmd;
        if (OLED_TxStart(0x00, &oledTxSingleByte, 1))
        {
            oledUpdateStep = OLED_UPDATE_STEP_SET_COL_HIGH;
        }
        break;
        case OLED_UPDATE_STEP_SET_COL_HIGH:
        cmd = 0x10 | ((oledUpdateX & 0xF0) >> 4);
        oledTxSingleByte = cmd;
        if (OLED_TxStart(0x00, &oledTxSingleByte, 1))
        {
            oledUpdateStep = OLED_UPDATE_STEP_SET_COL_LOW;
        }
        break;
        case OLED_UPDATE_STEP_SET_COL_LOW:
        cmd = 0x00 | (oledUpdateX & 0x0F);
        oledTxSingleByte = cmd;
        if (OLED_TxStart(0x00, &oledTxSingleByte, 1))
        {
            oledUpdateStep = OLED_UPDATE_STEP_SEND_DATA;
        }
        break;
        case OLED_UPDATE_STEP_SEND_DATA:
        if (OLED_TxStart(0x40, &OLED_DisplayBuf[oledUpdatePage][oledUpdateX], oledUpdateWidth))
        {
            oledUpdatePage++;
            oledUpdateStep = OLED_UPDATE_STEP_SET_PAGE;
        }
        break;
        default:
        oledUpdateStep = OLED_UPDATE_STEP_SET_PAGE;
...
static void OLED_ProcessCursor(void)
{
    ...
    switch (oledCursorStep)
    {
        case OLED_CURSOR_STEP_PAGE:
        cmd = 0xB0 | oledCursorPage;
        oledTxSingleByte = cmd;
        if (OLED_TxStart(0x00, &oledTxSingleByte, 1))
        {
            oledCursorStep = OLED_CURSOR_STEP_COL_HIGH;
        }
        break;
        case OLED_CURSOR_STEP_COL_HIGH:
        cmd = 0x10 | ((oledCursorX & 0xF0) >> 4);
        oledTxSingleByte = cmd;
        if (OLED_TxStart(0x00, &oledTxSingleByte, 1))
        {
            oledCursorStep = OLED_CURSOR_STEP_COL_LOW;
        }
        break;
        case OLED_CURSOR_STEP_COL_LOW:
        cmd = 0x00 | (oledCursorX & 0x0F);
        oledTxSingleByte = cmd;
        if (OLED_TxStart(0x00, &oledTxSingleByte, 1))
        {
            oledRunState = OLED_RUN_STATE_IDLE;
            oledCursorStep = OLED_CURSOR_STEP_PAGE;
            oledCursorPending = 0;
        }
        break;
        default:
...
static void OLED_RequestUpdate(uint8_t startPage, uint8_t endPage, uint8_t startX, uint8_t width)
{
    ...
    if (oledRunState != OLED_RUN_STATE_IDLE)
    {
        oledPendingStartPage = startPage;
        oledPendingEndPage = endPage;
        oledPendingX = startX;
        oledPendingWidth = width;
        oledUpdatePending = 1;
        return;
    }

    oledUpdateStartPage = startPage;
    oledUpdateEndPage = endPage;
    oledUpdateX = startX;
    oledUpdateWidth = width;
    oledUpdatePage = startPage;
    oledUpdateStep = OLED_UPDATE_STEP_SET_PAGE;
    oledRunState = OLED_RUN_STATE_UPDATE;
}
...
void OLED_Init(I2C_Regs* hi2c)
{
    ...
    DL_I2C_setTimeoutACount(I2C_OLED_INST, 0xFFFFF);
    DL_I2C_enableTimeoutA(I2C_OLED_INST);
    DL_I2C_enableInterrupt(I2C_OLED_INST, DL_I2C_INTERRUPT_TIMEOUT_A | DL_I2C_INTERRUPT_CONTROLLER_NACK | DL_I2C_INTERRUPT_CONTROLLER_ARBITRATION_LOST);
    NVIC_SetPriority(I2C_OLED_INST_INT_IRQN, 3);

    oled_i2c_handle = hi2c;			// 保存I2C句柄
    delay_cycles(3200000);					// 延时等待OLED供电稳定

    OLED_Clear();				// 清空显存数组

    oledTxState = OLED_TX_STATE_IDLE;
    oledRunState = OLED_RUN_STATE_INIT;
    oledInitIndex = 0;
    oledUpdatePending = 0;
    oledCursorPending = 0;

    OLED_RequestUpdate(0, 7, 0, 128);
    OLED_Task();
}
...
{
    if (hi2c == NULL || hi2c != oled_i2c_handle)
    {
        return;
    }
...
void OLED_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
    ...
    OLED_RequestUpdate((uint8_t)page, (uint8_t)(page1 - 1), (uint8_t)startX, (uint8_t)width);
}
