...
### 5.3 #include 顺序与最小包含原则

遵循 BARR-C:2018 最小包含原则：**`.h` 只 include 其接口直接使用的类型所需的头文件，其余全放 `.c`**。

- 标准库（如 `<stdio.h>`、`<string.h>`）几乎永远不需要出现在 `.h` 中——格式化、内存操作等是实现细节
- 其他模块的头文件（如 `"pwm.h"`）只有在 `.h` 的函数声明参数中直接引用该模块类型时才需要 include，否则放 `.c`
- 平台 HAL 头文件（如 `<stm32f4xx_hal.h>`）只需在 `.h` 中使用 `GPIO_TypeDef *`、`UART_HandleTypeDef *` 等平台类型时包含

`.c` 文件按以下顺序 include，组间不加空行：
...
```c
/* .c 文件 */
#include "motor_controller.h"   /* 1.自身 */
#include "pwm.h"                /* 2.其他模块 */
#include "sensor_adc.h"
#include <stdio.h>              /* 4.标准库 */
#include <string.h>
#include "stm32f4xx_hal.h"     /* 5.HAL */
```

```c
/* .h 文件 — 仅包含接口直接需要的头文件 */
#include <stm32f4xx_hal.h>     /* GPIO_TypeDef 等平台类型 */
#include <stdint.h>             /* uint16_t 等固定宽度类型 */
...
- 简短的中文行注释（`/* 按键消抖时间(ms) */`）可用于解释宏或类型，不必为每个声明单独加注释。
...
使用 `for (;;)` 或 `while (1)`，项目内统一即可。`for (;;)` 是嵌入式惯例，某些编译器对 `while (1)` 会产生条件判断警告。主循环默认使用 `while (1)`。
