...
以下所有条目均需对照 CODING_STANDARD.md v1.3 修改，完成后从清单中删除。

### 一、错误处理框架（待实现）

§12.9 方案已确认但代码未落实：

- [ ] **创建 `error_handler.h` / `error_handler.c`**：按 §12.9 规范实现三层架构（传输/上报/处理），含 `error_source_t` 枚举、`drv_err_t` 定义、队列缓冲、`switch(source)` 分派处理
- [ ] **删除 `st/drv_err.h` 和 `ti/drv_err.h`**：`drv_err_t` 合并入 `error_handler.h`，所有 `#include "drv_err.h"` 改为 `#include "error_handler.h"`
- [ ] **所有 `return DRV_ERR_*` 前加 `error_report()`**（44 处）：
  - st/: blueteeth.c:129, buzzer.c:44, cam.c:55, encoder.c:48, gyroscope.c:63, key.c:101, laser.c:31, led.c:50, motor.c:76, oled.c:60, pid.c:34, pwm.c:41, sensor/mcu/sensor.c:57, sensor/non-mcu/sensor.c:66, servo.c:102/114/206/233/248/251/271/322, step_motor.c:229, ultrasonic.c:67
  - ti/: blueteeth.c:169, buzzer.c:44, cam.c:65, gyroscope.c:69, key.c:98, laser.c:31, led.c:50, motor.c:93, oled.c:113, pid.c:34, pwm.c:49, sensor/mcu/sensor.c:54, sensor/non-mcu/sensor.c:66, step_motor.c:267, ultrasonic.c:79
- [ ] **所有空指针 early return 前加 `error_report()`**（82 处）：所有模块的 setter/getter/task/callback 函数中的 `if (!handle)` / `if (!huart)` / `if (!cfg)` 等空指针检查后直接 return 的路径，均需先调 `error_report()`
- [ ] **各模块添加 `error_source_t` 枚举条目**：每个需要上报错误的模块在 `error_handler.h` 的 `error_source_t` 枚举中增加对应条目
- [ ] **`error_process()` switch 补齐**：为每个 `error_source_t` 条目的 case 编写具体错误处理逻辑（重新初始化、复位等）

### 二、Bug 修复（待修复）

- [ ] **`ti/gyroscope.c` — `gyro_init()` 缺少 `return DRV_OK;`**（行 82）：函数声明返回 `drv_err_t`，成功路径未 return，导致未定义行为。参照 `st/gyroscope.c` 补齐
- [ ] **`ti/gyroscope.c` — `gyro_rx_callback()` 是 void 函数却 `return DRV_OK;`**（行 118）：删除返回值，改为裸 `return;`

### 三、代码风格违规（待修改）

- [ ] **`continue` 语句**（4 处，规范禁止）：`st/key.c:218`、`st/key.c:241`、`ti/key.c:216`、`ti/key.c:239`。用状态标志或重构循环逻辑替代
- [ ] **`ti/encoder.c:63` — `timer_capture_right_count` 应为 `encoder_capture_right_count`**：模块对外标识符需以模块名为前缀
- [ ] **冗余静态变量零初始化**：`st/encoder.c:68/91` 和 `ti/encoder.c:132/156` 的 `static ... = 0`，去掉 `= 0`（静态变量自动零初始化，显式赋零掩盖未初始化警告）
