...
- `menu.h` / `menu.c` / `menu_config.h` — OLED 按键菜单（ST/TI 共用，放根目录；参数配置集中在 menu_config.h）
...
- `ti/oled`、`st/oled` + `oled_data` — SSD1306 OLED（0.96 寸 I2C 128×64，ASCII 字模）
  - 双平台统一非阻塞 I2C 状态机：`oled_task` 每 tick 推进一个 I2C 操作，`oled_update` 仅置 pending 标志位
  - TI：`DL_I2C_fillControllerTXFIFO` + `startControllerTransfer`，轮询 `getControllerStatus` 判定完成
  - STM32：`HAL_I2C_Mem_Write_IT` 非阻塞启动，轮询 `HAL_I2C_GetState` 判定完成
...
- `menu` + `menu_config` — OLED 按键菜单（多级菜单 + 现场调参，ST/TI 共用，放根目录。与按键解耦：应用层做 key→action 映射；值体系统一 `int32_t` + `decimals`；动作即时响应 + menu_task_flag 节流渲染；内部已调 oled_clear/update，使用菜单时不另调 oled task；新增参数只需编辑 menu_config.h）
...
| I2C 非阻塞发送 | `DL_I2C_fillTXFIFO` + `startTransfer`，`getControllerStatus` 轮询完成 | `HAL_I2C_Mem_Write_IT`，`HAL_I2C_GetState` 轮询完成 |
...
单一上下文：`CONTEXT.md` + `docs/adr/` 在仓库根目录。详见 `docs/agents/domain.md`。
