...
这是一个嵌入式外设驱动库的参考代码集合。**旧代码已放入，待逐个按规范重写。**
...
- **12.2 统一调度**：定时器 ISR 统一管理所有模块的执行周期，`volatile` 标志位通知 task。A 类（标志位触发）用于复杂状态机，B 类（ISR 直接执行）用于极简读取
- **12.3 串口**：DMA + IDLE 中断 + 环形缓冲区（保留一个空位判别空/满）。TI 平台用 DMA 余量不变判定法替代硬件 IDLE
- **12.4 ISR**：只做读寄存器→置标志位→搬数据→清中断。中断优先级：控制类 > 通信类 > 系统 tick
- **12.5 状态机**：每个有状态外设一个主状态枚举 + `xxx_task()` 中 switch 推进，每个等待状态必须有超时兜底
- **12.6 模块接口**：`init()` → `task()` / 回调 的标准函数模式，返回值 0 成功 / 负值错误码
- **12.7 数据共享**：`static` 私有变量 + getter 函数暴露，禁止 `extern` 全局变量
- **12.8 单/多实例**：默认多实例（句柄注入），客观上唯一的硬件允许单实例
- **12.9 错误处理**：公开接口完整校验，内部函数轻量校验，ISR 最简校验。错误恢复：上报→复位→重试（限次数）→故障
- **12.11 错误上报**：蓝牙屏显 `display_show_error()`，一行就够了，不需要复杂日志框架
